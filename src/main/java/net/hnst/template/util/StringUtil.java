package net.hnst.template.util;
...
public class StringUtil {
     ...
     * 比较某天跟今天相差的天数
     *
     * @param timeStr     某天的时间字符串
     * @param partternStr 字符串格式 yyyyMMdd
     * @return 目标日期在今天之后 为 正数
     * 目标日期在今天之前 为 负数
     */
    public static int getDistanceDays(String timeStr, String partternStr) {
        //某天的0时0分0秒0毫秒
        DateTime nowDay = DateTime.now().withTime(0, 0, 0, 0);
        DateTime endDay = DateTime.parse(timeStr, DateTimeFormat.forPattern(partternStr)).withTime(0, 0, 0, 0);
        return Days.daysBetween(nowDay, endDay).getDays();
    }

    public static String formatReplaceAll(String time) {
